...
Actor → ActorOvertimeEntry[] (날짜+종류별 추가근무 시간)
...
- `ActorOvertimeEntry(actorId, date, type)` — 배우별 날짜+종류 중복 방지
...
- **API Route 인증 패턴**: `getToken()` (next-auth/jwt) 사용 금지. 반드시 `auth()` from `@/lib/auth` 사용 후 `session?.user?.role` 체크
...
| `/api/actors/overtime?year=&month=` | GET | ADMIN | 월별 추가근무 항목 조회 |
| `/api/actors/overtime` | POST | ADMIN | 추가근무 항목 upsert (actorId+date+type 기준) |
| `/api/actors/overtime/[id]` | DELETE | ADMIN | 추가근무 항목 삭제 |
...
MINIMUM_WAGE: Record<number, number>  // 연도별 최저시급 (교육 추가근무 단가)
EXTRA_SHOW_RATE = 15000              // 추가공연 시간당 단가 (원)
```
